...
function getUserDefinedMergedFieldDirectivesForDefinition(definitionNode) {
    ...
    for (const field of definitionNode.fields || []) {
        ...
        const matched = field.directives.filter((directive) => !(0, is_in_array_1.isInArray)(library_directives_1.LIBRARY_DIRECTIVES, directive.name.value));
        if (matched.length) {
...
function getUserDefinedDirectives(definitionNodes) {
    ...
    for (const definitionNode of definitionNodes.objectTypes) {
        const userDefinedObjectDirectives = definitionNode.directives?.filter((directive) => !(0, is_in_array_1.isInArray)(library_directives_1.LIBRARY_DIRECTIVES, directive.name.value)) ||
            [];
    ...
    for (const definitionNode of definitionNodes.interfaceTypes) {
        const userDefinedInterfaceDirectives = definitionNode.directives?.filter((directive) => !(0, is_in_array_1.isInArray)(library_directives_1.LIBRARY_DIRECTIVES, directive.name.value)) ||
            [];
        const propagatedDirectives = definitionNode.directives?.filter((directive) => (0, is_in_array_1.isInArray)(constants_1.PROPAGATED_DIRECTIVES, directive.name.value)) ||
            [];
    ...
    for (const definitionNode of definitionNodes.unionTypes) {
        const userDefinedUnionDirectives = definitionNode.directives?.filter((directive) => !(0, is_in_array_1.isInArray)(library_directives_1.LIBRARY_DIRECTIVES, directive.name.value)) ||
            [];
        const propagatedDirectives = definitionNode.directives?.filter((directive) => (0, is_in_array_1.isInArray)(constants_1.PROPAGATED_DIRECTIVES, directive.name.value)) ||
            [];
